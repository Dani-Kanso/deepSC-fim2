...
def train_step(model, src, trg, n_var, pad, opt, criterion, channel, mi_net=None, if_attack=False):
    model.train()
    ...
    # Apply FIM
    enc_output, importance_scores = model.fim(enc_output, src_mask, trg_real)
    ...
    # Channel decoding and prediction
    ...
    # Calculate losses
    ntokens = pred.size(-1)
    translation_loss = loss_function(pred.contiguous().view(-1, ntokens), 
    ...
    # Total loss
    loss = translation_loss + 0.1 * sparsity_loss
...
def val_step(model, src, trg, n_var, pad, criterion, channel, if_attack=False):
    model.eval()
    channels = Channels()
    trg_inp = trg[:, :-1]
    trg_real = trg[:, 1:]
